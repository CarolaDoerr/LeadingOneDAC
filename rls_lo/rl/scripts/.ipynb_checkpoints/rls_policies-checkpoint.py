...
class RLSOptimalPolicy:
    def __init__(self, env, name="RLSOptimal"):
        # TODO: not sure why the following one doesn't run
        #assert isinstance(env.unwrapped(), RLSEnv), "ERROR: RLSOptimalPolicy only works with RLSEnv environment"
        self.name = name
        self.env = env        
    def get_next_action(self, obs):
...
class RLSFixedOnePolicy:
    def __init__(self, env, name="RLSFixedOne"):
        # TODO: not sure why the following one doesn't run
        #assert isinstance(env.unwrapped(), RLSEnv), "ERROR: RLSOptimalAgent only works with RLSEnv environment"  
        self.name = name
        self.env = env        
    def get_next_action(self, obs):        
        return 1
...
class RLSOptimalDiscretePolicy:
    def __init__(self, env, name="RLSOptimalDiscrete"):
        #assert isinstance(env.unwrapped(), RLSDiscreteK), "ERROR: RLSOptimalAgent only works with RLSEnv environment"  
        self.name = name
        self.env = env
